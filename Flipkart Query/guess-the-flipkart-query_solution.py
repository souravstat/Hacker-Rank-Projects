...
for x in range(len(predicted)):
    ...
    if flag == 0:         
        if 'dell' not in tmp and ('laptop' in tmp or 'laptops' in tmp): print query[6]
        elif 'spoken' in tmp or 'Spoken' in tmp: print query[-5]
        elif 'written' in tmp or 'ibps' in tmp or 'bank' in tmp or 'story' in tmp or 'banking' in tmp or 'stories' in tmp: print query[-1]
        
        elif 'book' in tmp or 'books' in tmp: print query[1]
        elif '++' in tmp or 'c' in tmp: print query[7]
        elif 'combos' in tmp or 'combo' in tmp: print query[9]
        
        else: print query[predicted[x]]
        #print query[predicted[x]]
